        ...
        try:
            async with engine.begin() as conn:
                ...
                try:
                    ...
                    await conn.execute(text("ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS is_bot_link BOOLEAN DEFAULT FALSE"))
                    await conn.execute(text("ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS bot_id VARCHAR"))
                    await conn.execute(text("ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS bot_start_param VARCHAR"))
                    await conn.execute(text("ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS project_id VARCHAR"))
